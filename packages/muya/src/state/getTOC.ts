...
export function getTOC(muya: Muya): ITocItem[] {
    ...
    for (const node of scrollPage.children.iterator()) {
        ...
        const { superSubScript, footnote, texMathDollars, texMathGfm, texMathSingleBackslash } = muya.options;
        const content = tokensToPlainText(
            tokenizer(source, {
                hasBeginRules: false,
                options: { superSubScript, footnote, texMathDollars, texMathGfm, texMathSingleBackslash },
            }),
        ).trim();
