...
- This previews the most recent `pnpm run build` output via `electron-vite preview`, so it behaves like a production launch. It does **not** rebuild — re-run `pnpm run build:unpack` first if your sources changed.
