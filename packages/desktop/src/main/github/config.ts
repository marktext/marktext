...
// This is the literal shipped id — a maintainer fills it in when the OAuth
// App is registered. Empty means "not configured" (see requestDeviceCode).
...
// guard reflects the live value. Env override wins over the shipped id.
...
// Per-request network timeout for all GitHub HTTP calls, so a hung request
// never pins the UI's busy/spinner state forever.
