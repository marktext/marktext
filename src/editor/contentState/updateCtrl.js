  ...
  ContentState.prototype.updateState = function (event) {
    ...
    if (event.type === 'input' && oldStart.key !== oldEnd.key) {
      ...
      if (startBlock.type === 'pre') {
        ...
        return this.partialRender([ startBlock ])
      }
    }
    ...
    if (event.type === 'click' && block.type === 'figure' && block.functionType === 'table') {
      ...
      return this.partialRender([ cursorBlock ])
    }
    ...
    if (block && block.type === 'pre') {
      if (block.key !== oldKey) {
        ...
        if (event.type === 'click' && oldKey) {
          ...
          const outMostOfOldBlock = this.findOutMostBlock(oldBlock)
          needRenderBlocks.push(outMostBlock, outMostOfOldBlock)
    ...
    if (oldKey !== key || oldStart.offset !== start.offset || oldEnd.offset !== end.offset) {
      needRender = true
      if (key !== oldKey && oldBlock) {
        needRenderBlocks.push(this.findOutMostBlock(this.getBlock(oldKey)))
      }
    }
    ...
    if (checkMarkedUpdate || inlineUpdatedBlock || needRender) {
      const outMostBlock = this.findOutMostBlock(inlineUpdatedBlock || block)
