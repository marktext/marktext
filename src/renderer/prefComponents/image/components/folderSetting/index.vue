<template>
  <section class="image-folder">
    ...
    <text-box :description="$t('pref.image.folder.globalFolder')" :input="imageFolderPath"
    ...
    <div>
      ...
      <el-button size="mini" @click="openImageFolder">{{ $t('pref.image.folder.showInFolder') }}</el-button>
    </div>
    <compound>
      <template #head>
      ...
      <template #children>
        ...
        <div class="footnote" v-html="$t('pref.image.folder.relativeHint')">
        </div>
      </template>
    </compound>
  </section>
...
export default {
  ...
  computed: {
    ...
    relativeDirectoryNamePlaceholder: {
      get: function () {
        return this.$store.state.preferences.imageRelativeDirectoryName || this.$t('pref.image.folder.assets')
      }
    }
  },
  methods: {
