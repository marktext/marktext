<template>
  <div class="pref-keybindings">
    ...
    <section class="keybindings">
      <div class="text">
        {{ $t('pref.keybindings.description') }}
        {{ $t('pref.keybindings.availableOnline') }} <a class="link" @click="openKeybindingWiki">{{ $t('pref.keybindings.online') }}</a>.
      </div>
      <el-table
        :data="keybindingList"
        style="width: 100%"
      >
        <el-table-column prop="description" :label="$t('pref.keybindings.colDescription')">
        </el-table-column>
        <el-table-column prop="accelerator" :label="$t('pref.keybindings.colKeyCombo')" width="220">
        </el-table-column>
        <el-table-column fixed="right" :label="$t('pref.keybindings.colOptions')" width="90">
          <template slot-scope="scope">
            <el-button @click="handleEditClick(scope.$index, scope.row)" type="text" size="small" :title="$t('pref.keybindings.edit')">
              <i class="el-icon-edit"></i>
            </el-button>
            <el-button @click="handleResetClick(scope.$index, scope.row)" type="text" size="small" :title="$t('pref.keybindings.reset')">
              <i class="el-icon-refresh-right"></i>
            </el-button>
            <el-button @click="handleUnbindClick(scope.$index, scope.row)" type="text" size="small" :title="$t('pref.keybindings.unbind')">
    ...
    <section class="footer">
      <separator></separator>
      <el-button size="medium" @click="saveKeybindings">{{ $t('pref.keybindings.save') }}</el-button>
      <el-button size="medium" @click="restoreDefaults">{{ $t('pref.keybindings.restoreDefault') }}</el-button>
    </section>
    <section v-if="showDebugTools" class="keyboard-debug">
      <separator></separator>
      <div><strong>{{ $t('pref.keybindings.debugOptions') }}</strong></div>
      <el-button size="medium" @click="dumpKeyboardInformation">{{ $t('pref.keybindings.dumpKeyboardInfo') }}</el-button>
    </section>
    <key-input-dialog
...
export default {
  ...
  methods: {
      ...
      if (this.keybindingConfigurator && this.keybindingList.length > 0) {
        this.keybindingConfigurator.save()
          .then(success => {
            if (!success) {
              notice.notify({
                title: this.$t('pref.keybindings.failedSave'),
                type: 'error',
                message: this.$t('notification.unexpectedError')
              })
            }
          })
    ...
    handleDuplicateShortcut (id, accelerator) {
      notice.notify({
        title: this.$t('pref.keybindings.shortcutInUse'),
        type: 'warning',
        message: this.$t('pref.keybindings.shortcutInUseMsg', { accelerator })
      })
    },
