<template>
  <div class="pref-theme">
    ...
    <section v-show="false" class="import-themes ag-underdevelop">
      <div>
        <span>{{ $t('pref.theme.openThemesFolder') }}</span>
        <el-button size="small">{{ $t('pref.theme.openFolder') }}</el-button>
      </div>

      <div>
        <span>{{ $t('pref.theme.importCustomThemes') }}</span>
        <el-button size="small">{{ $t('pref.theme.importTheme') }}</el-button>
      </div>
    </section>
  </div>
</template>
...
export default {
  ...
  computed: {
    autoSwitchThemeOptions () {
      return autoSwitchThemeOptions(this.$t.bind(this))
    },
    ...mapState({
