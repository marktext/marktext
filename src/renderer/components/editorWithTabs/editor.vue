<template>
  <div class="editor-wrapper"
    ...
    <el-scrollbar class="editor-scroll"
      ref="editorScroll"
      wrapClass='scroll-wrap'>
      <div ref="editor"
        class="editor-component"
        :style="[ getEditorLineWidth ]"></div>
    </el-scrollbar>
    ...
    <el-dialog :visible.sync="dialogTableVisible"
      ...
      <el-form :model="tableChecker"
        :inline="true">
        <el-form-item label="Rows">
          <el-input-number ref="rowInput"
            size="mini"
...
export default {
  ...
  created () {
    this.$nextTick(() => {
      ...
      this.editor = new Muya(ele, options)
      ...
      this.editor.on('selectionChange', changes => {
        ...
        if (this.typewriter) {
          const { wrap } = this.$refs['editorScroll']
          animatedScrollTo(wrap, wrap.scrollTop + y - STANDAR_Y, 'top', {
            duration: 100
          })
        }
      ...
      this.editor.on('muya-scroll-to-position', () => {
  ...
  methods: {
    ...
    scrollToPosition () {
      ...
      const { end } = this.editor.contentState.cursor
      const anchor = document.querySelector('#' + end.key)
      if (anchor) {
        const { y } = anchor.getBoundingClientRect()
        // container.scrollTo(x, container.scrollTop + y - STANDAR_Y)
        const { wrap } = this.$refs['editorScroll']
        animatedScrollTo(wrap, wrap.scrollTop + y - STANDAR_Y, 'top', {
          duration: 150
        })
