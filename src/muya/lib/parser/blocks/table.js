...
function createTableBlock (item, stateRender) {
  ...
  for (i = 0; i < item.header.length; i++) {
    stateRender.appendChild(headRow, stateRender.createBlock('th', String(item.header[i]), {
      column: i,
      align: item.align[i]
    }))
  }
  ...
  for (i = 0; i < item.cells.length; i++) {
    ...
    for (j = 0; j < row.length; j++) {
      // TODO:parse td content inline
      stateRender.appendChild(tdHeadRow, stateRender.createBlock('td', String(row[j]), {
        column: i,
        align: item.align[i],
        attrs: {
          style: `text-align:${item.align[i]}`
        }
      }))
    }
