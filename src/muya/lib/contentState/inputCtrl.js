      ...
      if (
        ...
        if (/^delete/.test(event.inputType)) {
          ...
          if (
            ...
            // alphanumeric.
            !/\\/.test(preInputChar) &&
            ((autoPairQuote && /[']{1}/.test(inputChar) && !(/[a-zA-Z\d]{1}/.test(postInputChar)) && !(/[a-zA-Z\d]{1}/.test(preInputChar))) ||
              (autoPairQuote && /["]{1}/.test(inputChar) && !(/[a-zA-Z\d]{1}/.test(postInputChar))) ||
              (autoPairBracket && /[\{\[\(]{1}/.test(inputChar) && !(/[a-zA-Z\d]{1}/.test(postInputChar))) ||
